...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = query(
      'SELECT ps.*, r.route_name, s.subsection_name, e.name AS entity, c.checkpoint_name, u.email as user_email, u.name as user_name, rev.email as reviewer_email, rev.name as reviewer_name, ai.score as ai_score, ai.confidence as ai_confidence, ai.issues as ai_issues, ai.positives as ai_positives, ai.summary as ai_summary, ai.status as ai_status, ai.human_score as ai_human_score, ai.human_notes as ai_human_notes FROM photo_submissions ps LEFT JOIN routes r ON CAST(ps.route_id AS TEXT) = r.route_id LEFT JOIN subsections s ON CAST(ps.route_id AS TEXT) = s.route_id AND CAST(ps.subsection_id AS TEXT) = s.subsection_id LEFT JOIN checkpoints c ON ps.checkpoint_id = c.id LEFT JOIN entities e ON c.entity_id = e.id LEFT JOIN users u ON ps.user_id = u.id LEFT JOIN users rev ON ps.reviewer_id = rev.id LEFT JOIN photo_ai_scores ai ON ai.photo_submission_id = ps.id ' + whereClause + ' ORDER BY ps.created_at DESC LIMIT ? OFFSET ?',
      params
    );
