...
const allCards = [
  ...
  { href: '/patrol', title: 'Patrol', desc: 'Start route patrol', color: 'bg-teal-500', icon: iconSvg(['M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z', 'M15 11a3 3 0 11-6 0 3 3 0 016 0z']) },
  { href: '/patrol/manager', title: 'Patrol Manager', desc: 'Live map & analytics', color: 'bg-rose-600', icon: iconSvg(['M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7', 'M12 2a10 10 0 110 20 10 10 0 010-20z']) },
];

const PATROL_HREFS = ['/patrol', '/patrol/manager'];

function cardsForRole(role: string | undefined): typeof allCards {
  if (role === 'Engineer') {
    return allCards.filter((c) => c.href === '/capture');
  }
  if (role === 'Reviewer') {
    return allCards.filter((c) => c.href !== '/admin' && !PATROL_HREFS.includes(c.href));
  }
  if (role === 'Patroller') {
    return allCards.filter((c) => c.href === '/patrol');
  }
...
export default function DashboardPage() {
  ...
  const cards = cardsForRole(meData?.role);
